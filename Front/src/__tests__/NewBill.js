  ...
  describe("Integration Tests", () => {
    describe("When I do fill fields in correct format and I click on submit button", () => {
      test("Then the submission process should work properly, and I should be sent on the Bills Page", async () => {
        ...
        const imageInput = screen.getByTestId("file");
        const file = getFile(inputData.fileName, ['jpg', 'jpeg', 'png']);
        const fileValidation = jest.spyOn(newBill, "fileValidation");
        selectExpenseType(inputData.type);
        await userEvent.type(getExpenseName(), inputData.name);
        await userEvent.type(getAmount(), inputData.amount.toString());
        await userEvent.type(getDate(), inputData.date);
        await userEvent.type(getVat(), inputData.vat.toString());
        await userEvent.type(getPct(), inputData.pct.toString());
        await userEvent.type(getCommentary(), inputData.commentary);
        await userEvent.upload(imageInput, file);
        expect(selectExpenseType(inputData.type).validity.valueMissing).toBeFalsy();
        expect(inputData.date).not.toBe('');
